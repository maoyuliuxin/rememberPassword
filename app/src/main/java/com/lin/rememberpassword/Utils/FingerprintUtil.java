...
public class FingerprintUtil {
    ...
    public static  void callFingerPrint(final OnCallBackListenr listener){
        FingerprintManagerCompat managerCompat = FingerprintManagerCompat.from(MyApplication.getContext());
        if (!managerCompat.isHardwareDetected()){ //判断设备是否支持
            if (listener != null)
                listener.onSupportFailed();
            return;
        }
        KeyguardManager keyguardManager =(KeyguardManager)MyApplication.getContext().getSystemService(MyApplication.getContext().KEYGUARD_SERVICE);
